...
export const FundChart: React.FC<FundChartProps> = ({ dataset, viewMode = 'raw', lang }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="glass-panel p-6 border-white/10 shadow-3xl rounded-[1.5rem] backdrop-blur-3xl min-w-[240px]">
          <p className="text-slate-400 text-[10px] font-black mb-4 border-b border-white/5 pb-3 uppercase tracking-[0.3em] font-mono">{label}</p>
          <div className="space-y-4">
            {payload.map((item: any) => (
              <div key={item.dataKey} className="flex flex-col">
                <div className="flex items-center justify-between gap-8">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.3)]" style={{ backgroundColor: item.color }}></div>
                    <span className="text-white text-[11px] font-black tracking-widest uppercase">{item.name}</span>
                  </div>
                  <span className="text-indigo-400 font-mono font-black text-xs tabular-nums tracking-tighter">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em]">{t.audit}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setShowLabels(!showLabels)}
            className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 px-5 py-3 rounded-xl border ${showLabels ? 'glass-cta shadow-indigo-500/10' : 'glass-button-secondary text-slate-400'}`}
          >
            {showLabels ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            <span className="hidden sm:inline">{showLabels ? t.toggleHide : t.toggleShow}</span>
          </button>
          <div className="text-[10px] font-black text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-5 py-3 rounded-xl flex items-center gap-3 shadow-xl shadow-indigo-500/5 tracking-[0.2em] uppercase">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out space-y-8 mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Object.keys(annotations).sort().map(date => {
              ...
              return (
                <div key={date} className="glass-panel border-white/5 p-8 relative shadow-2xl hover:shadow-indigo-500/5 hover:border-white/10 transition-all rounded-[2rem] group flex flex-col backdrop-blur-3xl overflow-hidden active:scale-[0.99] duration-300">
                  <div className="absolute top-0 right-0 p-8 opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-700">
                    <MessageSquarePlus className="w-40 h-40" />
                  </div>
                  ...
                  <button
                    ...
                    className="absolute top-6 right-6 text-slate-600 hover:text-red-400 transition-all p-2 hover:bg-red-400/10 rounded-xl"
                  >
                    <X className="w-5 h-5" />
                  </button>
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <span className="font-black text-white text-xl tabular-nums tracking-tighter">{date}</span>
                  </div>
                    ...
                    {dataset.funds.map((fund, idx) => (
                      <div key={fund} className="flex flex-col">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <div className="w-2 h-2 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.2)]" style={{ backgroundColor: COLORS[idx % COLORS.length] }}></div>
                            <span className="text-slate-400 font-black text-[10px] uppercase tracking-widest truncate max-w-[140px]">{fund}</span>
                          </div>
                          <span className="font-mono font-black text-indigo-400 tabular-nums text-xs">
                            {point[fund] !== null ? (point[fund] as number).toLocaleString(undefined, { minimumFractionDigits: 2 }) : (
                              <span className="text-slate-700 italic opacity-50">NULL</span>
                            )}
                          </span>
                        </div>
                        {dataset.metadata?.[fund]?.description && (
                          <span className="text-[9px] text-slate-600 font-bold ml-4.5 mt-1 tracking-tight truncate">{dataset.metadata[fund].description}</span>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="relative mt-auto pt-6 border-t border-white/5">
                    <div className="flex items-center gap-3 mb-4 text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] bg-indigo-500/10 w-fit px-3 py-1 rounded-lg border border-indigo-500/10 shadow-lg">
                    ...
                    <textarea
                      ...
                      className="w-full glass-input p-4 text-sm text-white focus:ring-indigo-500 transition-all rounded-2xl resize-none h-28 placeholder:text-slate-600 leading-relaxed font-medium shadow-inner tracking-wide"
                    />
                  </div>
                </div>
              );
